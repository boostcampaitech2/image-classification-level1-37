...
class CustomAugmentation1:
    def __init__(self, height,width, mean, std, **args):
        self.transform = A.Compose([
            A.CenterCrop(height=height,width=width), #exp13
...
class CustomAugmentation3:
    def __init__(self, height,width, mean, std, **args):
        self.transform = A.Compose([
            A.Resize(height=height,width=width),
            A.CoarseDropout(min_holes = 50, max_holes=300,p=0.5,min_width=50), #CoarseDropout of the rectangular regions in the image.
            A.Rotate(limit=(-15,15),p=0.5),
            A.HorizontalFlip(p=0.5),
            A.Normalize(mean=mean, std=std),
            ToTensorV2(),
...
class MaskSplitByProfileDataset(MaskBaseDataset):
    ...
    def _split_profile(profiles, val_ratio):
        ...
        val_indices = set(random.choices(range(length), k=n_val)) #k 크기의 개수를 choices
