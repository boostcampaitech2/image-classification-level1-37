...
import math
target_model = SwinTransformer()
target_model.load_state_dict(torch.load('/opt/ml/model/swin_large_patch4_window7_224_22k.pth'), strict=False)
AGE_INPUT_NUM = 3
AGE_CLASS_NUM = 3
# for param in target_model.parameters():
#     param.requires_grad = False
# target model의 입력 크기와 출력 크기를 변경하여 줍니다 :) 새로운 네트워크 가중치를 만들어서 기존 부분 중 일부를 변경합니다.
#target_model.conv1 = torch.nn.Conv2d(AGE_INPUT_NUM, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
#target_model.fc = torch.nn.LazyLinear(out_features=AGE_CLASS_NUM, bias=True)
...
#labels = [DATA.encode_multi_class(mask, gender, age) for mask, gender, age in zip(dataset.mask_labels, dataset.gender_labels, dataset.age_labels)]
