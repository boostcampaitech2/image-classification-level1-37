...
from albumentations.augmentations.transforms import Cutout, HorizontalFlip, RGBShift, RandomBrightness, RandomBrightnessContrast
...
class CustomAugmentation2:
    def __init__(self, height,width, mean, std, **args):
        self.transform = A.Compose([
            ...
            A.CoarseDropout(max_holes=8, max_height=8, max_width=8, min_holes=None, min_height=None, min_width=None, fill_value=0, mask_fill_value=None, always_apply=False, p=0.5),
            A.Resize(height=height,width=width),
            A.Crop(x_min=80, y_min=50, x_max=300, y_max=370, always_apply=False, p=1.0),
            A.HorizontalFlip(p=0.5),
            ToTensorV2()
        ])
